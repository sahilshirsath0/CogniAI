...
// ✅ COMPLETE FIXED Verification Email Function
const sendVerificationEmail = async (email, code) => {
  try {
    console.log("📧 Sending to:", email);

    const result = await resend.emails.send({
      from: process.env.RESEND_FROM_EMAIL || "nadafaiwaz2911@gmail.com",
      to: email,
      ...
      html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f8fafc; border-radius: 12px;">
                    ...
                    <div style="background: white; padding: 30px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        <p style="font-size: 16px; text-align: center; margin-bottom: 20px;">Your verification code:</p>
                        <h1 style="letter-spacing: 8px; color: #1e293b; font-size: 42px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; margin: 20px 0; padding: 20px; border-radius: 12px;">${code}</h1>
                        <p style="text-align: center; color: #64748b; font-size: 14px;">Expires in <strong>10 minutes</strong></p>
                    </div>
                    <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e2e8f0;">
                        <p style="color: #6b7280; font-size: 14px;">Best,<br><strong>COGNI AI Team</strong></p>
                    </div>
                </div>
            `,
    });
    ...
    try {
      ...
      if (registrations.length === 0) {
        return res.status(400).json({
          message: "No confirmed registrations found to send certificates to.",
        });
      }
      ...
      res.json({
        message: `Process complete. Sent ${sentCount} certificates out of ${registrations.length} participants.`,
      });
    } catch (error) {
